...
BEGIN
    FOR i IN 1..array_length(p_column_ids, 1) LOOP
        UPDATE columns 
        SET position = i - 1 
        WHERE id = p_column_ids[i] AND project_id = p_project_id;
    END LOOP;
...
CREATE OR REPLACE FUNCTION get_column_tasks(p_column_id UUID)
RETURNS TABLE(
    id UUID,
    column_id UUID,
    title VARCHAR(500),
...
BEGIN
    ...
    SELECT 
        (SELECT COUNT(DISTINCT pm.project_id) 
         FROM project_members pm WHERE pm.user_id = p_user_id) as total_projects,
        (SELECT COUNT(*) FROM tasks t 
         JOIN columns c ON t.column_id = c.id
         JOIN project_members pm ON c.project_id = pm.project_id
         WHERE pm.user_id = p_user_id) as total_tasks,
        (SELECT COUNT(*) FROM tasks t 
         JOIN columns c ON t.column_id = c.id
         JOIN project_members pm ON c.project_id = pm.project_id
         WHERE pm.user_id = p_user_id AND LOWER(c.name) LIKE '%done%') as tasks_completed,
        (SELECT COUNT(*) FROM tasks t 
         JOIN columns c ON t.column_id = c.id
         JOIN project_members pm ON c.project_id = pm.project_id
         WHERE pm.user_id = p_user_id AND LOWER(c.name) LIKE '%progress%') as tasks_in_progress;
END;
$$ LANGUAGE plpgsql;
